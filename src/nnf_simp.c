...
int in_neg (tnode *t1, formulalist *l) {
  ...
  if (t1->type == NEGATION) {
    ...
    tnode *aux = create_tnode(NEGATION,NEGATION,t1,NULL,NULL);
    result = in(aux,l);
    free(aux);
  }
  return result;
...
int in_neg_box(tnode *t1, formulalist *l) {
  ...
  if (t1->left->type == NEGATION) {
    ...
    aux = create_tnode(NEGATION,NEGATION,t1->left,NULL,NULL);
    flag = 1;
  }
...
int in_neg_dia(tnode *t1, formulalist *l) {
  ...
  if (t1->type == NEGATION && t1->left->left->type == NEGATION ) //~box ~phi ==> aux = phi
    ...
    aux = create_tnode(NEGATION,NEGATION,t1->left->left,NULL,NULL);
    flag = 1;
  }
...
formulalist *simplify_and (formulalist *s) {
  ...
  if (s != NULL) {
    ...
    if (s->formula->type == NEGATION) {
      ...
      tnode *aux = create_tnode(NEGATION,NEGATION,s->formula,NULL,NULL);
